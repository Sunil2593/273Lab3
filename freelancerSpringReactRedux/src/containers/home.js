...
class Home extends Component {
    ...
    componentDidMount() {
        ...
        if(this.props.session.answer==="loggedOut")
        {
            history.push('/login');
        }
    ...
    handleProjectClick = (e,project) => {
        ...
        var projectDetails={projectId:project.project_id,projectName:project.project_name};
    ...
    getDashboard = (e) => {
        e.preventDefault();
        var userDetails={user_id:this.props.session.user_id};
    ...
    postProject = (e) => {
        ...
        history.push({
            ...
            state: { user_id: this.props.session.user_id }
        })
...
function mapStateToProps(state) {
    return {
        projects:state.loginsigup.data.projects,
        session:state.loginsigup.data.user[0]
    };
}
